...
import {
  fetchBlogArticleIndex, buildArticleCard
...
async function decorateArticleFeed(
  ...
  if (!articleCards) {
    articleCards = div({class: 'article-cards'})
    articleFeedEl.appendChild(articleCards);
  }
  // display spinner

  const emptyDiv = div({class: 'article-cards-empty'});
  ...
  if (articles.length) {
    ...
    spinner.remove();
    const noMatches = p();
    const userHelp = p({class: 'article-cards-empty-filtered'});
    emptyDiv.append(noMatches, userHelp);
  } else {
    // no results were found
    spinner.remove();
    const noResults = p();
    noResults.innerHTML = `<strong>No Results</strong>`;
